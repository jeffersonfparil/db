...
fn_append = function(df, database, table_name, verbose=TRUE) {
    ...
    vec_idx_numerics = which(unlist(lapply(df[1, ], FUN=function(x){!is.na(x) & is.numeric(x)})))
    vec_idx_strings = which(unlist(lapply(df[1, ], FUN=function(x){!is.na(x) & !is.numeric(x)})))
    if (verbose) {
