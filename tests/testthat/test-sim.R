...
test_that("fn_simulate_tables", {
    ...
    expect_equal(ncol(list_sim$df_phenotypes), n_traits+12)
    expect_equal(nrow(list_sim$df_environments), n_dates*n_sites*n_treatments*n_replications*n_sensors)
    expect_equal(ncol(list_sim$df_environments), n_abiotics+6)
    expect_equal(nrow(list_sim$df_genotypes), n_entries)
    expect_equal(ncol(list_sim$df_genotypes), n_loci+1)
    ...
    expect_equal(ncol(df_genotypes)-3, nrow(list_sim$df_genotypes))
    expect_equal(nrow(df_genotypes), ncol(list_sim$df_genotypes)-1)
})
